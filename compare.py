...
def centralize(template):
	...
	core_coord = np.array(singular_pts[0][0])
	minutiae_list_cent = []
	# print(">> Centralize")
	# print(core_coord)
...
def match(tempA, tempB):
	...
	print("label: ", tempA[0], " == ", tempB[0])
	print("img index: ", tempA[-3], " == ", tempB[-3])
	print(scores)
	if(tempA[1] != tempB[1]):
		scores[0] = 0
...
def minutia_match(pointsTypedA, pointsTypedB, threshold=16):
	...
	for type_no in range(len(pointsTypedA)):
		...
		# print("")
	if (match_score_pts == 0):
		match_score_pts = 1
	if (points == 0):
		points = 1
	if (mse_threshold_pts == 0):
		mse_threshold_pts = 1
	match_score = match_score/(match_score_pts)
	match_score_total = match_score/(points)
	mse = mse/(points*2)
	mse_threshold = mse_threshold/(mse_threshold_pts*2)
	return [match_score, match_score_total, mse, mse_threshold]
