...
if __name__ == "__main__":
	...
	images, subject_nos, singular_pts_list_true = load.fingerprints(
		"DB/Rindex28", blk_sz)
	# images, subject_nos, singular_pts_list_true = load.fingerprintsL("DB/Lindex101", blk_sz)
	...
	index_start = 0
	step = 1
	index_end = len(images)  # exclusive
	print("(len(images[0:]))")
	print(len(images[0:]))
	...
	distances = stats.distanceMatrix(finger_templates, compare.match)
	labels = subject_nos[index_start:index_end:step]
	...
	for metric_i in range(distances.shape[0]):
		...
		fpsList, fnsList = stats.curveFARFRR(
			metric_distance_matrix, labels)

		stats.DETCurve(fpsList, fnsList)
		plt.savefig('DETCurve'+str(metric_i)+'.png')
		# Equal Error Rate
		# where difference is minimal
		diff=np.abs(np.array(fpsList) - np.array(fnsList))
		argEER=np.argmin(diff)

		print("fps_"+str(metric_i), fpsList[argEER])
		print("fns_"+str(metric_i), fnsList[argEER])

	id_accuracy = stats.identification(distances, labels)
